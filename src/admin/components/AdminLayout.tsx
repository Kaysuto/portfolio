...
export const AdminLayout: React.FC<AdminLayoutProps> = ({ children }) => {
  ...
  const getGravatarUrl = (email: string, size: number = 40) => {
    if (!email) return `https://www.gravatar.com/avatar/00000000000000000000000000000000?s=${size}&d=mp&r=g`;
    
    // Utilisation de crypto-js pour un vrai hash MD5
      ...
      <nav className={cn(
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-3 sm:py-4">
          <div className="flex items-center justify-between">
            ...
            {/* Right side actions */}
            <div className="flex items-center space-x-2 sm:space-x-3 animate-slideInFromRight animate-delay-100 flex-shrink-0">
              {/* User Info - Desktop (static, non-cliquable) */}
              <div className="hidden md:flex items-center space-x-3 px-3 py-2 bg-accent/10 rounded-xl border border-accent/20">
                {/* Avatar Gravatar */}
