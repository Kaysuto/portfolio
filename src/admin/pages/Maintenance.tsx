...
// Service fictif pour la maintenance
const MaintenanceService = {
  getMaintenanceConfig: async (): Promise<MaintenanceConfig> => {
    return {
      is_enabled: false,
      message: 'Le site est temporairement en maintenance pour améliorer votre expérience.',
      estimated_time: '2 heures'
    };
  },
  
  updateMaintenanceConfig: async (config: MaintenanceConfig): Promise<void> => {
    console.log('Updating maintenance config:', config);
  }
};
...
export default function Maintenance() {
  ...
  const loadMaintenanceConfig = async () => {
    ...
    try {
      const maintenanceConfig = await MaintenanceService.getMaintenanceConfig();
      if (maintenanceConfig) {
        setConfig(maintenanceConfig);
        setFormData({
          is_enabled: maintenanceConfig.is_enabled,
          message: maintenanceConfig.message || '',
          estimated_time: maintenanceConfig.estimated_time || ''
        });
      }
    } catch (err) {
      console.error('Erreur chargement config maintenance:', err);
  ...
  const handleSave = async () => {
    ...
    try {
      await MaintenanceService.updateMaintenanceConfig({
  ...
  const toggleMaintenance = async () => {
    const newStatus = !formData.is_enabled;
    setFormData({ ...formData, is_enabled: newStatus });
          ...
          <div className="flex items-center justify-between bg-background/40 backdrop-blur-md border border-border/50 rounded-xl p-6">
            <div>
              ...
              <p className="text-muted-foreground">
                {formData.is_enabled ? 'Site en maintenance' : 'Site opérationnel'}
              </p>
            </div>
            <div className="flex items-center gap-4">
              <div className="flex items-center gap-2">
                ...
                <span className={`font-medium ${formData.is_enabled ? 'text-red-500' : 'text-green-500'}`}>
                  {formData.is_enabled ? 'Maintenance' : 'En ligne'}
                </span>
              </div>
              <Button
                onClick={toggleMaintenance}
                variant={formData.is_enabled ? "destructive" : "default"}
                disabled={isSaving}
              >
                {formData.is_enabled ? 'Désactiver' : 'Activer le mode'}
              </Button>
            </div>
          </div>
